...
- Dedicated artifact-server smoke coverage is split by capability. The operator GitHub Kind workflow installs the pinned `HTTPRoute` CRD before operator startup, provides `mlflow-artifacts-tls`, enables `ARTIFACTS_SERVER=true` for one PostgreSQL/PostgreSQL/S3 row, and port-forwards the artifact Service to test workspace-authenticated upload, list, download, and multipart operations directly. Downstream OpenShift additionally sets `ARTIFACTS_SERVER_GATEWAY=true` to wait for route acceptance and verify tracking-relative rewrites from artifact pod access logs. Both tests carry the `smoke` and `artifacts_server` markers; Kind skips only the Gateway-specific test.
