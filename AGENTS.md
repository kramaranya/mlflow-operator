...
- Dedicated artifact-server compatibility coverage is opt-in because it requires a real OpenShift Gateway controller: run `mlflow-tests/images/test-run.sh -m artifacts_server` with `ARTIFACTS_SERVER=true`, one S3 backend, and PostgreSQL backend/registry stores. The test sends bearer-authenticated, workspace-scoped UI list/download requests and a real SeaweedFS multipart create/abort flow through `/mlflow`, then verifies the rewritten paths in artifact pod access logs. Do not add this row to Kind-only matrices unless they install and configure a Gateway controller capable of reaching the chart's TLS backends.
